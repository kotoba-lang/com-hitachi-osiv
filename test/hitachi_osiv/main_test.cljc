(ns hitachi-osiv.main-test
  "Contract + behavioral test for the hitachi-osiv L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "hitachi_osiv-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'hitachi-osiv.main-test)]
