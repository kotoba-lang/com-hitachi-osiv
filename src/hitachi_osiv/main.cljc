(ns hitachi-osiv.main
  "Kotodama WASM entrypoint for the Hitachi_osiv clean-room actor (L4) — Clojure port.
  ...
  facts (`hitachi_osiv.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hitachi_osiv")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"hitachi_osiv.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hitachi_osiv-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hitachi_osiv-compat/ready)
